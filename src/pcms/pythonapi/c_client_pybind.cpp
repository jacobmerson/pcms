...
#include <pcms/capi/client_cpp.h>
...
template <typename T>
PcmsFieldAdapterHandle xgc_field_adapter_numpy(const char* name, MPI_Comm comm, 
py::array_t<T> data, const PcmsReverseClassificationHandle rc, in_overlap_function in_overlap) {
    ...
    pcms_create_xgc_field_adapter_t<T>(name, comm, buffer_info.ptr, buffer_info.size, *reverse_classification, in_overlap, *field_adapter);
...
PYBIND11_MODULE(c_pcms_python, m)
{
    ...
    m.def("pcms_create_xgc_field_adapter",&pcms::xgc_field_adapter_numpy<double>, "Create an XGC field adapter for double");
    m.def("pcms_create_xgc_field_adapter",&pcms::xgc_field_adapter_numpy<float>, "Create an XGC field adapter for float");
    m.def("pcms_create_xgc_field_adapter",&pcms::xgc_field_adapter_numpy<int>, "Create an XGC field adapter for int");
    m.def("pcms_create_xgc_field_adapter",&pcms::xgc_field_adapter_numpy<long int>, "Create an XGC field adapter for long int");
