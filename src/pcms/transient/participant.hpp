...
//
// This is Step 1 of that plan's incremental roadmap (§11): the Participant /
// Coupling / strategy contracts plus concrete, dependency-free implementations
// that drive a two-subdomain overlapping-Schwarz problem in one process. The
// SUNDIALS-backed strategies named in the plan (SundialsTimestepper wrapping
// SUNAdaptController; KinsolAnderson wrapping KINSOL+Anderson) are left as
// clearly marked stubs behind PCMS_ENABLE_SUNDIALS and replaced here by
// hand-rolled equivalents so the whole control stack is exercised without the
// dependency (plan §9.4).
...
// Layer 0 — the only thing a code implements. A thin, non-invasive adapter
// wrapping EITHER a local solver (in-process, the near-term target) OR an
// Application comms-proxy to a remote executable (MPMD; not in this first
// draft). The transient layer never invents a new field type — it drives the
// interface transmission data the adapter already owns.
//
// In-process vs. redev deployment (plan §6, open question §10.1): this draft
// takes the direct in-process path — GetInterface/SetInterface move host
// InterfaceStates and the "transfer" is applied directly (Coupling below). The
// redev path would instead expose Participant::Interface(name) ->
// FunctionHandle<Real> and route through Application Send/Receive phases +
// TransferHandle::Run(); that RemoteParticipant lands in plan Step 6.
class Participant
{
public:
